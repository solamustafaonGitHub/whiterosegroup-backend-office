...
const userProfileSchema = new Schema({
    ...
    userProfilePhoneNo: { type: Schema.Types.ObjectId, ref: 'ApplicationUser', required: true },
    ...
    userProfileDOB: { type: Date },
    userProfileWorkStatus: {
        type: String,
        required: true,
        enum: ['EMPLOYED', 'NOT CURRENTLY EMPLOYED'],
    },
    userProfileSelectCompany: {
...
userProfileSchema.pre('save', async function (next) {
    try {
        if (this.userProfilePhoneNo) {
            const appuser = await ApplUser.findById(this.userProfilePhoneNo).exec();
            if (appuser) {
                this.UserProfileEmail = appuser.applUserEmail;
            }
        }
        next();
    }
