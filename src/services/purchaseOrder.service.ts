...
function drawSectionCCell(x: number, y: number, width: number, height: number, text: string, isHeader = false) {
...
function drawCell(x: number, y: number, width: number, height: number, text: string, isHeader = false) {
...
function drawRow(values: string[], columnWidths: number[], rowY: number) {
    let currentX = tableX;
    let maxRowHeight = 0;
...
function drawSectionDRow(values: string[], event: any) {
let currentX = tableX;
let maxRowHeight = 0;
    ...
    function calculateBalance(event: any): string {
    let balance: string;
...
//populate the 'Balance' column begins here
...
type PurchaseOrder = {
    id: string;
    remittanceBalanceToBePaidDetails?: RemittanceDetail[];
};

function getRemittanceExpectedBalanceToBePaidOnPO(purchaseOrder: PurchaseOrder): string {
...
function handlePurchaseOrderForPDF(purchaseOrder: PurchaseOrder) {
