...
import {SchemeInformation, ISchemeIP, ISchemePoolDetails} from '../models/schemeInformationProfile.model.js';
...
export const SchemeInformationPDFController = async (req:Request, res:Response) => {
    ...
    try {
        ...
        const schemeInformationObject = schemeInformation.toObject() as ISchemeIP;
...
async function generateSchemeInformationPDF(outputFilePath:string, schemeInformation:ISchemeIP): Promise<void> {
    ...
    //------------------Draw a Rectangle A around the Scheme Information---------------------------------------
    ...
    //---------------------Draw a Rectangle B around the scheme pool details & add currency formatting-------------------------------------------
    ...
    //------------------Draw a Rectangle B2 around the scheme pool details & add currency formatting--------------------------------
        ...
        //function to draw a cell in the table.
        function drawSectionB2Cell(x, y, width, height, text, isHeader = false) {x = x || 0; y = y || 0; width = width || 50; height = height || 20;
            doc.rect(x, y, width, height).stroke();
            doc.font(isHeader ? 'Helvetica-Bold' : 'Helvetica').fontSize(8.0);
            const padding = 5;
            doc.text(text || 'N/A', x + padding, y + padding, {width: width - padding * 2, align:'left', lineBreak:true});
    ...
    //------------------Draw a Rectangle C around Scheme Pool Details----------------------------------------------------
        ...
        // Function to draw a cell in the table
        function drawSectionCCell(x, y, width, height, text, isHeader = false) {x = x || 0; y = y || 0; width = width || 50; height = height || 20;
            doc.rect(x, y, width, height).stroke();
            doc.font(isHeader ? 'Helvetica-Bold' : 'Helvetica').fontSize(8.0);
            const padding = 5;
            doc.text(text || 'N/A', x + padding, y + padding, {width: width - padding * 2, align:'left', lineBreak:true});
        ...
        function drawSectionCValuesRow() {
            valuesSectionC.forEach((values) => {
                ...
                values.forEach((value, index) => {
                    // Ensure the value is defined and convert to a string safely
                ...
                // Check for page space and add new page if needed
                const availableSpace = doc.page.height - doc.y - doc.page.margins.bottom;
                if (maxRowHeight > availableSpace) {
                    doc.addPage();
                    tableYSectionC = doc.page.margins.top; // Reset table Y position for the new page
                    drawSectionCHeaderRow(); // Redraw the header row on the new page
                }
                ...
                tableYSectionC += maxRowHeight; // Update the Y position for the next row
            });
        }
    ...
    //------------Draw rectangle D to display the No. of Available Units-----------------------------------------------------
    const headerSectionD = [150, 415];
    doc.y = tableYSectionC + 30;
    // Add title for Total Units Available For Scheme
    ...
    //Function to draw a cell in the table
    function drawSectionDCell(x, y, width, height, text, isHeader = false) {x = x || 0; y = y || 0; width = width || 50; height = height || 20;
        doc.rect(x, y, width, height).stroke();
        doc.font(isHeader ? 'Helvetica-Bold' : 'Helvetica').fontSize(8.0);
        const padding = 5;
        doc.text(text || 'N/A', x + padding, y + padding, {width: width - padding * 2, align:'left', lineBreak:true});
    ...
    function drawSectionDHeaderRow() {
        ...
        if (maxHeaderHeight > availableSpace) {
            doc.addPage();
            tableYSectionD = doc.page.margins.top; // Reset table Y position for the new page
        }
        // Draw header cells
        headingsSectionD.forEach((header, index) => {
        ...
        if (maxRowHeight > availableSpace) {
            doc.addPage();
            tableYSectionD = doc.page.margins.top; // Reset table Y position for the new page
            drawSectionDHeaderRow(); // Redraw the header row on the new page
        }
        // Draw value cells
        valuesSectionD.forEach((value, index) => {
            ...
            tableYSectionD += maxRowHeight; // Move Y position for the next row
        };
        // Draw the table with header and rows
        ...
        //handleSchemeInformationForPDF(schemeInformation);
